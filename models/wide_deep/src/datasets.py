...
class H5Dataset:
    ...
    def __init__(
        self, data_path, train_mode=True, train_num_of_parts=21, test_num_of_parts=3
    ):
        ...
        self.data_size = self._bin_count(
            self._hdf_data_dir, self._file_prefix, self._num_of_parts
        )
    ...
    def _bin_count(self, hdf_data_dir, file_prefix, num_of_parts):
        ...
        for part in range(num_of_parts):
            y = pd.read_hdf(
                os.path.join(
                    hdf_data_dir, file_prefix + "_output_part_" + str(part) + ".h5"
                )
            )
            size += y.shape[0]
        return size
    ...
    def _generator(self, x, y, batch_size, shuffle=True):
        ...
        :return:
        """
        number_of_batches = np.ceil(1.0 * x.shape[0] / batch_size)
        counter = 0
        finished = False
    ...
    def batch_generator(
        self, batch_size=1000, random_sample=False, shuffle_block=False
    ):
        """
        :param train_mode: True or false,false is eval_mode,
        :param batch_size
...
def _padding_func(batch_size, manual_shape, target_column, field_size=39):
    ...
    if manual_shape:
        ...
        def padding_func(x, y, z):
            ...
            x_id = (
                np.ones((batch_size, target_column - field_size), dtype=np.int32)
                * filled_value
            )
            x_id = np.concatenate([x, x_id.astype(dtype=np.int32)], axis=1)
            mask = np.concatenate(
                [y, np.zeros((batch_size, target_column - 39), dtype=np.float32)],
                axis=1,
            )
            return (x_id, mask, z)
...
def _get_tf_dataset(
    ...
    data_set = data_set.map(
        operations=_padding_func(batch_size, manual_shape, target_column),
...
def _get_mindrecord_dataset(
    ...
    data_set = data_set.batch(int(batch_size / line_per_sample), drop_remainder=True)
    data_set = data_set.map(
        _padding_func(batch_size, manual_shape, target_column),
        input_columns=["feat_ids", "feat_vals", "label"],
...
def _get_vocab_size(
    ...
    new_vocabs = inidival_vocabs + [1] * (target_column_number - len(inidival_vocabs))
    ...
    if per_vocab_size is not None:
        ...
        if multiply is True:
            ...
            new_vocab_size = [
                math.ceil(int(item * k) / worker_size) * worker_size
                for item in new_vocab_size
            ]
            new_vocab_size = [(item // 8 + 1) * 8 for item in new_vocab_size]

        else:
            if total_vocab_size > sum(new_vocab_size):
                new_vocab_size[-1] = total_vocab_size - sum(new_vocab_size[:-1])
                # pylint: disable=R1721
                new_vocab_size = [item for item in new_vocab_size]
            else:
                raise ValueError(
                    f"Please providede the correct vocab size, now is {total_vocab_size}"
                )
...
def create_dataset(
    ...
    if data_type == DataType.MINDRECORD:
        return _get_mindrecord_dataset(
            data_dir,
            train_mode,
            batch_size,
            line_per_sample,
