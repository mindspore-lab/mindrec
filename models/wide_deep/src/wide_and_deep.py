    ...
    def __init__(self, config):
        ...
        if parameter_server:
            ...
            if dynamic_embedding:
                ...
                self.deep_embeddinglookup = HashEmbeddingLookup(
                    embedding_size=self.emb_dim,
                    vocab_cache_size=config.vocab_cache_size,
                )
                self.wide_embeddinglookup = HashEmbeddingLookup(
                    embedding_size=1, vocab_cache_size=config.vocab_cache_size
                )
            else:
...
class TrainStepWrap(nn.Cell):
    ...
    def __init__(
        ...
        if (
            (sparse and is_auto_parallel)
            or (sparse and parameter_server)
            or dynamic_embedding
        ):
